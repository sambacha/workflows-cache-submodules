...
class TestGitSubmoduleCaching(unittest.TestCase):
    ...
    def test_parse_gitmodules(self):
        with unittest.mock.patch('your_script_name.Repository', side_effect=[self.repo, self.submodule1, self.submodule2]):
            with unittest.mock.patch('your_script_name.discover_repository', side_effect=fake_discover_repository):
                expected_submodules = [
